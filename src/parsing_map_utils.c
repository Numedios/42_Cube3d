...
/*   Updated: 2023/05/29 18:14:05 by zhamdouc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_wall_line(t_game *game, int j, int last_line)
{
	...
	while (game->map[0] && game-> map[0][j])
	{
		if (game->map[0][j] != ' ' && game->map[last_line]
			&& game->map[0][j] != '1')
		{
			printf("ERROR :\nmap premiere line pas composer uniquement de 1\n");
			free_game_exit(game, 1);
		}
		if (game->map[last_line][j] != ' ' && game->map[last_line][j] != '1')
		{
			printf("ERROR :\nmap derniere line pas composer uniquement de 1\n");
			free_game_exit(game, 1);
		}
		j++;
	}
...
void	southcheck_adjacent_four(int row, int col, t_game *game)
{
...
void	check_adjacent_three(int row, int col, t_game *game)
{
	...
	southcheck_adjacent_four(row, col, game);
}
...
void	check_wall_map(t_game *game)
{
	...
	while (game->map && game->map[i])
	{
		...
		while (game->map && game->map[i] && game->map[i][j])
		{
			...
			if (game->map[i][j] == '0')
				check_adjacent_three(i, j, game);
			j++;
		}
		if (game->map[i][(game->map_p.max_widht - 1)] == '0')
		{
			printf("ERROR :\n%s\nline non fermer a la fin\n", game->map[i]);
			free_game_exit(game, 1);
		}
		i++;
	}
